...
class _DeliveryOrderScreenState extends State<DeliveryOrderScreen> {
  ...
  void _loadData() {
    ...
    if (token != null) {
      context.read<DeliveryOrderProvider>().fetchDeliveryOrders(
        token,
  ...
  Widget _buildHeader() {
    return Column(
      ...
      children: [
        ...
        Text(
          'Lacak status pengiriman dan konfirmasi penerimaan barang.',
          style: GoogleFonts.poppins(
            fontSize: 14,
                    ...
                    child: DropdownButton<String>(
                      ...
                      items:
                          ['Created', 'In Transit', 'Delivered', 'Cancelled']
                              .map(
                                (s) => DropdownMenuItem(
                                  value: s.toLowerCase().replaceAll(' ', '_'),
                                  child: Text(s),
                                ),
                              )
                              .toList()
  ...
  Widget _buildDataTable() {
    return Consumer<DeliveryOrderProvider>(
      builder: (context, provider, _) {
        ...
        if (provider.deliveryOrders.isEmpty) {
          return Center(
            child: Column(
              children: [
                const Icon(
          ...
          child: Column(
            children: [
              Container(
                ...
                child: Row(
                  children: [
                    Expanded(flex: 3, child: _buildTableHeader('NO. DO')),
              ...
              ListView.separated(
                ...
                itemCount: provider.deliveryOrders.length,
                separatorBuilder: (context, index) => const Divider(height: 1),
                itemBuilder: (context, index) {
                  final item = provider.deliveryOrders[index];
                  return InkWell(
                    onTap: () async {
                      await Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) =>
                              DeliveryOrderDetailScreen(doId: item.id),
                        ),
                      );
                      if (!mounted) return;
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            flex: 2,
                            child: Text(
                              DateFormat('dd/MM/yy').format(item.deliveryDate),
                              style: GoogleFonts.poppins(
                                fontSize: 12,
                                color: const Color(0xFF6B7280),
                              ),
